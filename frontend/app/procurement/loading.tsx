...
export default function ProcurementDraftsLoading() {
  return (
    <main style={{ padding: spacing[6], fontFamily: typography.fontFamily, backgroundColor: colors.background, minHeight: '100vh' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: spacing[6] }}>
        <div>
          <div style={{ background: colors.background, borderRadius: borderRadius.sm, height: 28, width: 160, marginBottom: spacing[2] }} />
          <div style={{ background: colors.background, borderRadius: borderRadius.sm, height: 14, width: 220 }} />
        </div>
      </div>
      <div style={{ background: colors.surface, borderRadius: borderRadius.base, border: `1px solid ${colors.border}`, padding: spacing[5] }}>
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} style={{ background: colors.background, borderRadius: borderRadius.sm, height: 72, marginBottom: spacing[2] }} />
        ))}
      </div>
    </main>
  );
