...
class TestFeedbackRecordEndpoints:
    ...
    def test_create_feedback(self, client):
        ...
        # First create a batch and item
        batch_resp = client.post(
            "/api/v1/batches",
            json={"store_id": "test-store", "trigger_type": "manual"},
        )
        batch_id = batch_resp.json()["data"]["id"]

        item_resp = client.post(
            "/api/v1/opportunities",
            json={"batch_id": batch_id, "store_id": "test-store"},
        )
    ...
    def test_list_feedback_with_filters(self, client):
        ...
        response = client.get(
            "/api/v1/feedback-records",
            params={"feedback_type": "content_fix", "review_status": "pending"},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
    ...
    def test_approve_feedback(self, client):
        ...
        # Create batch, item, and feedback
        batch_resp = client.post(
            "/api/v1/batches",
            json={"store_id": "test-store", "trigger_type": "manual"},
        )
        ...
        fb_resp = client.post(
            ...
            json={
                "opportunity_item_id": item_id,
                "feedback_type": "mapping_fix",
                "source_type": "reviewer",
                "before_payload": '{"category": "old"}',
                "after_payload": '{"category": "new"}',
            },
        )
        feedback_id = fb_resp.json()["data"]["id"]

        # Approve
        response = client.post(f"/api/v1/feedback-records/{feedback_id}/approve")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
    ...
    def test_reject_feedback(self, client):
        ...
        fb_resp = client.post(
            ...
            json={
                "opportunity_item_id": item_id,
                "feedback_type": "pricing_fix",
                "source_type": "admin",
                "before_payload": '{"price": 100}',
                "after_payload": '{"price": 120}',
            },
        )
        feedback_id = fb_resp.json()["data"]["id"]

        # Reject
        response = client.post(f"/api/v1/feedback-records/{feedback_id}/reject")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
    ...
    def test_approve_nonexistent_feedback(self, client):
        ...
        response = client.post("/api/v1/feedback-records/nonexistent-id/approve")
    ...
    def test_list_demand_signals_for_item(self, client):
        ...
        client.post(
            ...
            json={
                ...
                "payload": '{"competitor_id": "12345"}',
                "snapshot_time": "2024-01-01T00:00:00Z",
            },
        )

        # List
        response = client.get(f"/api/v1/demand-signals/item/{item_id}")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
    ...
    def test_get_demand_signal(self, client):
        ...
        signal_resp = client.post(
            ...
            json={
                ...
                "payload": '{"pain_point": "quality issues"}',
                "snapshot_time": "2024-01-01T00:00:00Z",
            },
        )
        ...
        # Get
        response = client.get(f"/api/v1/demand-signals/{signal_id}")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
...
class TestTrainingPackageEndpoints:
    ...
    def test_create_training_package(self, client):
        ...
        response = client.post(
            ...
            json={
                ...
                "manifest_payload": '{"items": 100, "date": "2024-01-01"}',
            },
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_create_training_package_with_batch(self, client):
        ...
        response = client.post(
            ...
            json={
                "batch_id": batch_id,
                "package_type": "batch",
                "storage_uri": "s3://bucket/batch-package.tar.gz",
                "manifest_payload": '{"batch_id": "' + batch_id + '"}',
            },
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_list_training_packages(self, client):
        ...
        client.post(
            ...
            json={
                "package_type": "manual",
                "storage_uri": "s3://bucket/test.tar.gz",
                "manifest_payload": '{"test": true}',
            },
        )

        response = client.get("/api/v1/training-packages")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
        assert isinstance(data["data"], list)
    ...
    def test_get_training_package(self, client):
        ...
        pkg_resp = client.post(
            ...
            json={
                "package_type": "manual",
                "storage_uri": "s3://bucket/get-test.tar.gz",
                "manifest_payload": '{"test": "get"}',
            },
        )
        pkg_id = pkg_resp.json()["data"]["id"]

        # Get
        response = client.get(f"/api/v1/training-packages/{pkg_id}")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
    ...
    def test_get_nonexistent_training_package(self, client):
        ...
        response = client.get("/api/v1/training-packages/nonexistent-id")
        assert response.status_code == 404
